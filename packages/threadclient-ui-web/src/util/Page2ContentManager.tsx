...
export default class Page2ContentManager {
    ...
    setData(new_data: Generic.Page2Content) {
        // diff:
        batch(() => {
            const old_deleted = new Set<Generic.Link<unknown>>();
            for(const key of this.#signals.keys()) {
                old_deleted.add(key);
            }
            const in_new = new Set<Generic.Link<unknown>>();
            for(const key of Object.keys(new_data) as Generic.Link<unknown>[]) {
                in_new.add(key);
    ...
    view<T>(link: Generic.Link<T>): null | Generic.ReadLinkResult<T> {
        const signal = this.getSignal(link);
        return signal[0]() as Generic.ReadLinkResult<T>;
    }
    getSignal(link: Generic.Link<unknown>): Signal<null | Generic.ReadLinkResult<unknown>> {
...
function toLinkres(v: Generic.Page2Content[Generic.Link<unknown>]): null | Generic.ReadLinkResult<unknown> {
